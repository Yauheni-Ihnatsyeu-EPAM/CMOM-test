...
export default function Currency({route}: NativeStackScreenProps<RootTabParamList, 'Currency'>) {
  ...
  const renderCurrency = (data: CurrencyInfo) => {
    ...
    return (
      ...
      <Text style={{fontSize: 48}}>{data.name} <Text  style={{fontSize: 38, fontStyle: 'italic'}}> {data?.symbol}</Text></Text>
      
      <Text style={{fontSize: 30}}>hashing algo {data?.hashing_algorithm}</Text>
      <Text style={{fontSize: 30}}>market cap {data?.market_data.market_cap?.eur} €</Text>

      <Text style={{fontSize: 30}}>genesis date: {data?.genesis_date}</Text>

      <Text onPress={(event) => {linkPress(data.links.homepage[0])}} style={{fontSize: 30, color: 'blue'}}>Homepage </Text>
